...
export default function LauncherPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        const path = getAuditCollectionPath('production', user.uid);
        ...
        const rawSites = extractLauncherSites(data.repositories);
        const layout = await getLauncherLayout(user.uid, !!isAnonymous);
  ...
  const handleMove = async (index: number, direction: -1 | 1) => {
    ...
    // Save
    setSaving(true);
    await saveLauncherLayout(user!.uid, !!isAnonymous, newSites.map(s => s.id));
    setSaving(false);
  };
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      if (!snap.empty) {
        const rawSites = extractLauncherSites(snap.docs[0].data().repositories || []);
        setSites(rawSites);
        await saveLauncherLayout(user.uid, !!isAnonymous, rawSites.map(s => s.id));
      }
    } catch (e) {
      console.error(e);
    }
    ...
    return (
      <div className="flex flex-col h-full bg-slate-50 text-slate-900 font-sans p-6 items-center justify-center">
        ...
        <p className="text-slate-600 mb-6 text-center max-w-sm">
          {user ? 'Run an audit from the dashboard to populate the launcher.' : 'You must be logged in to view the launcher.'}
        </p>
          ...
          {sites.map((site, index) => (
            <div key={site.id} className="bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-md transition-shadow p-5 flex flex-col group relative">
              ...
              <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-slate-100">
                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-slate-100 text-slate-700 truncate max-w-[140px]" title={new URL(site.url).hostname}>
