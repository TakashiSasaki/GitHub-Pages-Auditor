# Full Report Table UI Design (v1.7.28)

Version: `1.7.28` (Full Report Table UI Stabilization)
...
- **The Solution**: All filters must participate only in a high-density toolbar positioned *above* or *outside* the table's scrollable view boundary. The actual table headers contain only text labels and inline tooltip action buttons.

---

## 5. Header Model
- **Unconstrained Header Height**: Table headers are given a sticky layout to stay pin-locked on vertical scrolling, but their heights must not be artificially capped with hard-coded second-row coordinate values (e.g., avoiding hard-coded `top-[31px]` offsets).
- **Sticky Layout Alignment**: If any multi-row header element is ever introduced in future phases, the second rows must compute dynamic positions using `ResizeObserver` or inline calculations rather than fixed pixel assumptions to prevent overlaps.
...
## 7. Virtualization and Scroll Consolidation
