...
- Do not describe GitHub OAuth or GitHub App as "future work", "not used in legacy V1", or "could be added later".
...
- Milestone 1.6.2 (Organization Scan Contract & V1 Erasure Baseline) is fully completed. All core backend, shared, export, anonymous lifecycles, and path modules are hardened and verified via automated test suites.
...
- Removed all obsolete legacy V1 references and schemas.
- 1.6.2 acts as the fixed development-complete baseline; future work is maintenance-only unless a new milestone is explicitly opened.
...
- **Primary deployment target**: **Cloud Run** (Active, Live).
  ...
  - Current Major Milestone: Organization Scan Contract & V1 Erasure Baseline (1.6.2)
...
- Advanced primary build threshold to `1.6.2 (Organization Scan Contract & V1 Erasure Baseline)` with patch version governance and public no-auth E2E validation controls.
